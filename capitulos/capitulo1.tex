...
Para el desarrollo de la Plataforma de Análisis Estadístico, se seleccionaron tecnologías modernas que aseguran que el sistema sea rápido, seguro y fácil de usar. A continuación, se comparan las herramientas y versiones específicas elegidas frente a opciones anteriores o alternativas en el mercado, justificando su implementación de una manera accesible.

\textbf{El lado visual de la plataforma (Frontend)}

Para construir la interfaz con la que interactúan los directivos, se eligió \textbf{React en su versión 18}. A diferencia de su versión anterior (React 17), la versión 18 introduce la capacidad de pausar y reanudar tareas visuales pesadas en segundo plano sin congelar la pantalla. Esto significa que cuando el usuario carga cientos de gráficas al mismo tiempo, la página sigue respondiendo de manera fluida. Aunque existen alternativas como Angular, React se destaca por trabajar como bloques de construcción, permitiendo armar pantallas complejas de manera ordenada y rápida.

En lugar de usar herramientas tradicionales de diseño como Bootstrap, que muchas veces hacen que todas las páginas web se vean iguales, se optó por \textbf{Tailwind CSS}. Esta herramienta nos da la libertad de crear un diseño visual único y moderno sin tener que cargar archivos estéticos innecesarios que volverían lenta la página.

Para mostrar los datos estadísticos, se integró \textbf{Apache ECharts}. Mientras que otras opciones como Chart.js son buenas para gráficos sencillos, ECharts es capaz de manejar grandes cantidades de información sin que la pantalla se congele. Esto es fundamental cuando se revisan años de historial académico, asegurando que las gráficas sean fluidas e interactivas.

\textbf{El cerebro del sistema (Backend y Base de Datos)}

En la parte interna que procesa la información, se utilizó \textbf{Python en su versión 3.12} mediante la herramienta \textbf{FastAPI}. En el mercado existen lenguajes como PHP o Node.js, pero Python es líder mundial para análisis estadísticos. Específicamente, dar el salto a Python 3.12 representa una mejora enorme frente a versiones pasadas (como la 3.9 o 3.10), ya que es considerablemente más veloz procesando datos matemáticos y consume mucha menos memoria del servidor. Combinado con FastAPI y \textbf{SQLAlchemy 2.0} (que ahora permite hacer múltiples conexiones a la vez de forma automática frente a su antigua versión 1.4), el sistema puede atender a muchos usuarios simultáneamente organizando tareas pesadas sin bloquearse.

Para guardar toda esta información, se seleccionó \textbf{PostgreSQL en su versión 16} como base de datos. Si bien existen opciones más famosas como MySQL, PostgreSQL es el estándar profesional por ser extremadamente seguro y confiable. Elegir la versión 16 frente a las versiones 14 o 15 nos permite aprovechar nuevas mejoras en la velocidad con la que el sistema busca y filtra millones de registros, garantizando que los reportes se generen en tiempo récord. Además, protege la integridad de los historiales estudiantiles para que nunca se pierdan o mezclen.

Finalmente, para que todo el sistema funcione de manera estable en internet, se utilizaron herramientas de contención como \textbf{Docker} y \textbf{Nginx}, apoyados por \textbf{Redis 7} para organizar los mensajes internos de forma ultra rápida. Estas opciones son el estándar actual porque actúan como barreras de seguridad y orden, asegurando que el sistema opere las 24 horas del día sin interrupciones, sin importar cuántas personas lo utilicen a la vez.